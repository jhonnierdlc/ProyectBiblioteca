...
import { actualizarPrestamo, obtenerPrestamoPorId } from "../../../services/prestamoServices";
import { useParams, useNavigate } from 'react-router-dom';

function EditarUsuario() {
    const [data, setData] = useState({ cedula: "", nombre: "", celular: "", libro: { titulo: "" }, fecha_inicio: "", fecha_devolucion: "" });
    const { id } = useParams();
        ...
        async function fetchData() {
            try {
                ...
                const updatedData = {
                    ...
                    fecha_inicio: new Date(prestamo.fecha_inicio).toISOString().split('T')[0],
                    fecha_devolucion: new Date(prestamo.fecha_devolucion).toISOString().split('T')[0],
                };
                setData(updatedData);
            } catch (error) {
                toast.error(error.response?.data || 'Error al obtener datos del préstamo');
                console.error(error);
            }
        }
    ...
    return (
        <div className="bdy">
            <div className="containerr">
                <div className="title">Editar Préstamo</div>
                <form onSubmit={handleSubmit}>
                    <div className="user-details">
                        ...
                        <div className="input-box">
                            ...
                            <input type="text" placeholder="Escriba el título del libro" name="libro.titulo" value={data.libro.titulo} readOnly />
                        </div>
                        <div className="input-box">
                            <span className="details">Fecha Préstamo</span>
                            <input type="date" placeholder="Digite la fecha de inicio" required name="fecha_inicio" value={data.fecha_inicio} onChange={handleChange} />
                        </div>
                        <div className="input-box">
                            <span className="details">Fecha Devolución</span>
                            <input type="date" placeholder="Digite la fecha de devolución" required name="fecha_devolucion" value={data.fecha_devolucion} onChange={handleChange} />
                        </div>
                    </div>
